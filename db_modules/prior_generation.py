...
def generate_prior_images(
    ...
    try:
        ...
        print(f"\n检查类别图像目录: {class_images_dir}, 找到 {existing_images} 张图像")
    except Exception as e:
        print(f"检查类别图像目录时出错: {e}")
        existing_images = 0
    
    # 如果没有足够的类别图像，则生成新图像
    if existing_images < num_samples * 0.1:  # 如果图像数量少于要求的10%，则生成新图像
        print(f"\n类别图像不足(只有{existing_images}张)，需要生成{num_samples}张图像用于先验保留...")
        
        # 禁用安全检查器以加速生成
        if hasattr(pipeline, 'safety_checker') and pipeline.safety_checker is not None:
            pipeline.safety_checker = None
            print("已禁用安全检查器以加速图像生成")
